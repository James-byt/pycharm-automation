...
def create(conn, results):
    ...
    print("Create_report")
    cursor = conn.cursor()
    sql = "insert into reports_noon ( Currency, Date, Results, Balance) values (%s,%s,%s,%s)"
...
def retrieve_matrix(conn):
    print()
    cursor = conn.cursor()
    cursor.execute("select * from goose_a_noon")
...
def add_submatrix(conn):
    ...
    cursor = conn.cursor()
    sql = "INSERT INTO goose_a_noon (a,b,c) VALUES (%s, %s, %s)"
...
def delete_submatrix(conn):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'delete from goose_a_noon LIMIT 1'
    )
    conn.commit()
...
while stop_number < 3:
    ...
    goal = "EURUSD" #EURUSD-OTC NZDUSD EURJPY AUDUSD
    new_money = 10
    ...
    def above_order_zone():
        ...
        # This is done so that when the second last rsi changes the if statement becomes not become false thus we validated from the second to the third
        if above_zone == 0 and second_last_rsi >= rsi_upper: #and second_lats_stoch >= stoch_upper:
            above_zone = 1
            #print("1st buy zone")
        if above_zone == 1 and third_last_rsi <= rsi_upper: #and third_lats_stoch <= stoch_upper:
            above_zone = 2
            #print("2nd buy zone")
