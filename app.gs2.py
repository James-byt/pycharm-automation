...
goal = "AUDUSD-OTC" # NZDUSD EURJPY AUDUSD
new_money = 10
...
while trade < 3:
    ...
    while keep_trading:
        ...
        if buy_zone == 3 or sell_zone == 3:
            ...
            if buy_zone == 3:
                print("buy wall")
                check,id = API.buy_digital_spot(ACTIVES, money, "put", duration)
                ...
                print("sell wall")
                check,id = API.buy_digital_spot(ACTIVES, money, action, duration)
                print("buy")
                results = API.check_win_v3(id)
                print(results)
